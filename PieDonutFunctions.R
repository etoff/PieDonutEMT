...
makeSubColor2=function(main,no=3){
  ...
  for(i in 1:length(main)){
    #print(i) #
    #print(ztable::gradientColor(main[i],n=no+4))
    temp=ztable::gradientColor(main[i],n=no+4)[4:(no+3)] #edit is here. 
    result=c(result,temp)
  }
  result
}
...
PieDonut2=function(data,mapping,
                  ...
                  donutCols=NULL,pieCols=NULL,
                  printdonutCols=FALSE, printpieCols=FALSE,
...
if(!isFALSE(printpieCols)){
  ...
  if(!is.null(donuts)){
    ...
    #donutCols edit EMT
    if(!is.null(donuts)){
      if(is.null(donutCols)){
        subColor=makeSubColor2(mainCol,no=length(unique(data[[donuts]])))
        subColor
      }else
        subColor=donutCols
    }
    if(!isFALSE(printdonutCols)){
      print("Donut colors are:")
      print(subColor)
    }
